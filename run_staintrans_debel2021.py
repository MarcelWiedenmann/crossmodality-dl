"""
Train the stain transfer baseline from De Bel et al. (2021).
...
from staintrans import debel2021
...
def train(opts):
    ...
    for i, (terminal_train_images, terminal_val_images, *_) in cv_folds:
        experiment_name = "staintrans_debel2021_"
        ...
        with Experiment(name=experiment_name, seed=c.seed, dry_run=opts.dry_run) as exp:
            ...
            model = debel2021.CycleGan(
                cycle_mask_quantile=opts.cycle_mask_quantile,
            ...
            training = Training(
                ...
                TrainingConfig(
                    ...
                    # The paper does not specify a maximum number of epochs but lets the model converge by learning rate
                    # decay. Choose a number of epochs after which the learning rate should virtually be zero.
                    epochs=165,
...
if __name__ == "__main__":
    ...
    # The original paper does not specify a batch size. Use the batch size that saturates the available GPU memory by
    # default (based on the default tile size of 256 above).
    train_parser.add_argument("--batch_size", type=int, default=44)
    train_parser.add_argument(
        "--no_cycle_mask", action="store_false", dest="cycle_mask"
    )
    train_parser.add_argument("--cycle_mask_quantile", type=float, default=0.1)
    train_parser.add_argument(
        "--generator", type=str, choices=["debel2019", "debel2021"], default="debel2021"
    )
    train_parser.add_argument(
    ...
    info("Arguments of staintrans debel2021 run:\n" + pformat(vars(args)))
